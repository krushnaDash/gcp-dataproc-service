...
import dp.{ClusterProps, DPCluster, DPJob}
import gcs.GCS
...
object DPGCS extends ZIOAppDefault with ApplicationLogger {
  ...
  private val libs = List("gs://bua-spark-udp-stg/bk-nrt-aggregator.jar")
  private val conf = Map("spark.executor.memory" -> "1g", "spark.driver.memory" -> "1g")

  private val mainClass = "com.walmart.bofbua.bookkeeping.Application"

  private val createCluster = DPCluster.createDataproc(dpCluster,
    ClusterProps(dpBucket
      ,subnetUri = Some("projects/shared-vpc-admin/regions/us-central1/subnetworks/prod-us-central1-03")
      , workerNumInstance = 2))

  private val program1 = DPJob
    .executeSparkJob(List("stage.conf", "NxnLolqanYf4raQBeTfPVgGZdqN9dG8AK1KHzjQaQP1nP4NBal3GC5TWdjT3JfWp2MwFM4QtfKKbr13yVOYjNg==", "5799.US", "2023-02-01"), mainClass, libs, conf)
